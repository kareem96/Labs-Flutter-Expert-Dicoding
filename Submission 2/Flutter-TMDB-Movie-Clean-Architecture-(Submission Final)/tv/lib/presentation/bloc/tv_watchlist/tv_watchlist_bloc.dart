...
class TvWatchListBloc extends Bloc<TvWatchListEvent, TvWatchListState>{
  ...
  TvWatchListBloc(this._getWatchlistTv, this._getWatchlistStatusTv, this._removeWatchlistTv, this._saveWatchlistTv) : super(TvWatchListInitial()){
  ...
  FutureOr<void> _onFetchTvWatchList(OnFetchTvWatchList event, Emitter<TvWatchListState> emit)async {
    emit(TvWatchListLoading());
    final result = await _getWatchlistTv.execute();
    result.fold((failure){
      emit(TvWatchListError(failure.message));
    }, (success){
      success.isEmpty ? emit(TvWatchListEmpty()) : emit(TvWatchListHasData(success));
    });
  }

  FutureOr<void> _onTvWatchListStatus(TvWatchListStatus event, Emitter<TvWatchListState> emit)async  {
    final id = event.id;
    final result = await _getWatchlistStatusTv.execute(id);
    emit(TvWatchListIsAdded(result));
  }

  FutureOr<void> _onTvWatchListAdd(TvWatchListAdd event, Emitter<TvWatchListState> emit) async{
  ...
  FutureOr<void> _onTvWatchListRemove(TvWatchListRemove event, Emitter<TvWatchListState> emit) async{
    final tv = event.tvDetail;
